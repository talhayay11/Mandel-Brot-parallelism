...
def compute_mandelbrot_region(xmin, xmax, ymin, ymax, width, height, max_iter):
    paralelStartTime = time.time()
    ...
    paralelEndTime = time.time()
    print('Time for',thread_id,":", paralelEndTime - paralelStartTime)
...
class ZoomableMandelbrot:
    ...
    def plot_mandelbrot(self):
        ...
        regions = [
            (self.xmin, x_mid, self.ymin, y_mid),  # Bottom-left
            (x_mid, self.xmax, self.ymin, y_mid),  # Bottom-right
            (self.xmin, x_mid, y_mid, self.ymax),  # Top-left
            (x_mid, self.xmax, y_mid, self.ymax)   # Top-right
        ]
        ...
        processesNumber = 2
        pool = multiprocessing.Pool(processesNumber)
        
        # Compute Mandelbrot set for each region in parallel
        results = pool.starmap(compute_mandelbrot_region, [(xmin, xmax, ymin, ymax, self.width // 2, self.height // 2, self.max_iter) for xmin, xmax, ymin, ymax in regions])
